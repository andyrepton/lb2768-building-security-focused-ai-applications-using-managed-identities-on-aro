...
So far, for this {ic-lab}, we have used the model https://aws.amazon.com/nova/[Amazon Bedrock Nova Pro,window=_blank]. It's easy to use and available to us as an API, but what if we try a smaller model running on only CPUs? Would that work? Let's try!
