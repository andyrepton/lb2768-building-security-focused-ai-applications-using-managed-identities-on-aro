...
- The **database**: a PostgreSQL database, running on the managed service RDS for PostgreSQL. It stores the claims.
- The **LLM**: the language model used to summarize and extract information from the claims. We are using https://aws.amazon.com/nova/[Amazon Bedrock Nova Pro,window=_blank] and consume it using the API.
- The **VectorDB**: the vector database used to store documents for retrieval in the RAG part. We have already prepared a Vector Database using https://docs.aws.amazon.com/prescriptive-guidance/latest/choosing-an-aws-vector-database-for-rag-use-cases/vector-db-options.html[AWS OpenSearch,window=_blank], where we have stored the content of the https://www.dmv.ca.gov/portal/handbook/california-driver-handbook/[California Driver's Handbook,window=_blank].
