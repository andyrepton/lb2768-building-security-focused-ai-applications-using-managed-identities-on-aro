...
In this {ic-lab} we are using a https://aws.amazon.com/rosa/[ROSA (Red Hat OpenShift Service on AWS),window=_blank] cluster, a native AWS service, with https://www.redhat.com/en/technologies/cloud-computing/openshift/openshift-ai/[Red Hat OpenShift AI,window=_blank] installed. We are going to use https://aws.amazon.com/bedrock[AWS Bedrock,window=_blank], Bedrock provides a large language model that we will use to summarize and extract information from fictive insurance claims. We are then also going to use https://aws.amazon.com/opensearch-service[Amazon OpenSearch Service,window=_blank], it is a vector database that can be used to store documents for retrieval when implementing RAG (Retrieval Augmented Generation). For our test application we are going to use https://docs.aws.amazon.com/rds/[Amazon RDS for PostgreSQL,window=_blank] to store our fictive insurance claims data.
...
In the AI on AWS Red Hat OpenShift workshop, you will explore how a fictional insurance company, Parasol Insurance uses generative AI capabilities on Red Hat OpenShift Service on AWS (ROSA) to improve its claims processing. In this immersive experience, you will have the opportunity to deploy and work with different AI models while utilizing Red Hat OpenShift AI and AWS Bedrock.

*Acceptable Use of Demo Environment*

You are accessing Red Hat OpenShift Service on AWS, Red Hat OpenShift AI and AWS Bedrock via the Red Hat Demo Platform that is operated by Red Hat. You are not using your own environment or applications during this demo.
